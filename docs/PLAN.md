...
client_packages   — id, client_id FK → clients.id, package_id FK → service_packages.id,
                     ...
                     (records a client's purchase of a package, e.g. "3-session
                      ...
                      form refuses that package; pick another package or make it ad-hoc);
...
bookings          — id, gcal_event_id, client_id, client_package_id?,
                     ...
                     (client_package_id links this booking to a specific package
                      purchase, consuming one session from it; nullable for ad-hoc
                      sessions not tied to a tracked package;
...
- **Overbooking hard-block:** select a package with 0 sessions remaining in Add-Booking → it's refused (must pick another package or go ad-hoc); a package at 0 stays `active` (no auto-`completed`).
