...
**Daily processing — EOD / SOD (Step 4.5d; design refined 2026-06-29):** bookings don't self-resolve, so resolution is a deliberate human habit, surfaced as a self-healing queue in the admin panel (no cron / background job — purely UI-driven, fits Vercel serverless).
> **Scope note (2026-06-28):** since the Google Calendar write-API is **deferred** (§12), the **GCal-retry step of EOD/SOD is NOT built in the current phase.** EOD/SOD ships with outcome-resolution + reminders only; the GCal retry sweep (and the per-booking "Retry calendar sync" button) come back if/when the write-API is switched on.

**One screen, not two (decided 2026-06-29):** EOD and SOD share most items, so both collapse into a **single `/admin/daily` ("Daily processing") page** built of sections that **auto-hide when empty** (the page only shows what needs action). Header = item count + a **Refresh** button (re-runs the server queries so cleared items drop off after you act elsewhere); "All clear ✓" empty state. Reached from a **Dashboard Staff card** + the **SOD overdue banner**. Every row carries its **own server-computed deep-link** (dynamic per item). Status-flip items get **inline one-tap buttons**; items needing more work get a **jump button** to the relevant record. Sections:
- **Bookings awaiting an outcome** — `confirmed AND scheduled_date ≤ today` (today's rows = EOD work; older rows = the overdue backlog / SOD safety net — same query, self-healing: resolve one and it drops off, skip one and it reappears next load). Inline **Completed / Cancelled / No-show**. New booking is `confirmed` ("booked"); **status never auto-flips.**
- **Send tomorrow's reminders** *(SOD)* — tomorrow's `confirmed` bookings with `reminder_sent_at` null; **one-tap** "Send via WhatsApp" opens `wa.me` (§11) **and** stamps `reminder_sent_at` so the row drops off (we accept fire-and-forget — wa.me can't confirm send). **Moved from EOD to SOD (decided 2026-06-29)** so any "can we reschedule?" replies land during working hours, not at night.
- **Upcoming collisions** *(SOD)* — overlaps among upcoming bookings (booking×booking, booking×calendar-block; reuse `src/lib/overlap.ts`). Jump → `/admin/bookings/day/<date>`. Should be fixed at booking time but may be missed/postponed; this is the catch.
- **Completed sessions missing a note** *(EOD+SOD, **admin-only**)* — `completed` bookings with no `session_notes` row. Jump → the booking's note page. (Gated to admins per the session-notes-admin-only rule.)
- **Exhausted packages to close** *(EOD+SOD)* — `active` package with 0 remaining; inline **"Mark completed"** (sets `client_packages.status = completed`). **Kept manual (decided 2026-06-29)** — not auto-flipped, because a later cancellation of a consuming booking would push `sessions_remaining` back above 0 and diverge from a `completed` status; a human confirms instead.
- **Unused sessions, no upcoming booking** *(SOD)* — client has an `active` package with sessions left but **no future `confirmed` booking** (includes a brand-new just-purchased package not yet booked). Jump → the client page to rebook.
- **Unpaid packages** *(SOD)* — `client_packages.paid = false`. Jump → the package page for payment follow-up.

**Self-heal note:** collisions / missing-notes / unused-sessions / unpaid are pure **derived queries** — they vanish automatically when the underlying record is fixed anywhere in the app, so the daily page is just a spotlight. Only outcome-resolution (`status`) and reminders (`reminder_sent_at`, added 2026-06-29) need stored state. **Excluded:** GCal retry (§12), calendar-block resolution (forward planning, nothing to resolve), no-show messaging (deliberate, §11).
...
- [ ] Admin: Bookings manager (add/edit/delete; venue + delivery_type + duration_minutes; **hard-block on exhausted package**; ~~GCal mirror push + `gcal_sync_failures` retry~~ **GCal deferred — §12**), with a DB-sourced **month→day calendar view** (bookings ∪ blocks; working-hours shading white/pink; click-to-create pre-fills the form) + **inline soft overlap warning** + **EOD/SOD processing** (resolve outcomes, day-before reminders; **no GCal retry in current build**)
  - 4.5a (CRUD + overlap/GCal-stub seam) **DONE**, 4.5b (month→day calendar) **DONE**, 4.5c (wa.me confirmation + copy-button panel) **DONE**, **4.5d (Daily processing / EOD+SOD) DONE** + many UI-refinement rounds (see `docs/PROGRESS.md`). 4.5d = single `/admin/daily` page (design locked 2026-06-29; no GCal retry) across 5 steps: migration `Booking.reminder_sent_at`; `src/lib/daily.ts` (pure section logic); `daily/actions.ts` (resolveOutcome / markReminderSent / markPackageCompleted); the page + `DailyButtons.tsx` (7 auto-hiding sections, Refresh, "All clear ✓"); Dashboard "Daily processing" card + SOD overdue banner. **All of Step 4.5 is DONE.** NEXT = thorough manual test on preview-dev, then 4.6 (Vitest + auth-hardening)
...
| Booking outcome processing (EOD/SOD) | **Decided 2026-06-20; design refined 2026-06-29 — build = Step 4.5d.** New bookings are `confirmed` ("booked"); **status never auto-flips**. Now a **single `/admin/daily` page** (not two screens) of auto-hiding sections + Refresh button + dynamic per-row deep-links — see §7 for the full section list. Reminders **moved to SOD** (sent for tomorrow's bookings; one-tap = wa.me + stamp `reminder_sent_at`). Exhausted packages → **manual** "Mark completed" (not auto). Adds collisions / missing-notes (admin-only) / unused-sessions / unpaid sections. SOD safety net = a Dashboard banner of overdue (`confirmed`, past-dated) bookings that **re-surfaces until resolved**. **No GCal retry** (deferred). Booking an **exhausted package (0 remaining) is HARD-BLOCKED**. |
